...
// Handle opportunity form submission
        document.getElementById('opportunityForm').addEventListener('submit', function(e) {
            ...
            const oppId = oppIdInput ? oppIdInput.value : '';
            ...
            const maxApplicantsInput = document.getElementById('oppMaxApplicants');
            const maxApplicants = maxApplicantsInput ? maxApplicantsInput.value.trim() : '';
            ...
            const url = oppId ? `/api/opportunities/${oppId}/edit` : '/api/opportunities';
            ...
            .catch(err => showToast('Error submitting form'));
        });
            ...
            .then(data => {
                ...
                if (data.data && data.data.length > 0) {
                    ...
                    data.data.forEach(opp => {
                        ...
                        const applicantsCount = opp.maxApplicants ? `${opp.maxApplicants} applicants` : '0 applicants';
                        card.innerHTML = `
                            <div class="opportunity-card-header">
                                ...
                                <div class="opportunity-meta">
                                    ...
                                    <span><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>${escapeHtml(opp.startDate)}</span>
                                </div>
                            </div>
                            <p class="opportunity-description">${escapeHtml(opp.description)}</p>
                            <div class="opportunity-skills"><div class="opportunity-skills-label">Skills You'll Gain</div><div class="skills-tags">
                            ...
                            <div class="opportunity-footer" style="display:flex; justify-content:space-between; align-items:center;">
                                ...
                                <div style="display:flex; gap:8px;">
                                    <button class="view-course-btn" style="width: auto; padding: 4px 12px; background: var(--qf-orange); border:none; border-radius:4px; color: white; cursor: pointer;" onclick="editOpportunity(${opp.id})">Edit</button>
                                    <button class="view-course-btn" style="width: auto; padding: 4px 12px; background: #e74c3c; border:none; border-radius:4px; color: white; cursor: pointer;" onclick="deleteOpportunity(${opp.id})">Delete</button>
                                    <button class="view-course-btn detail-btn" style="width: auto; padding: 4px 12px;">Details</button>
                                </div>
                            </div>
                        `;
                        
                        const viewBtn = card.querySelector('.detail-btn');
                        viewBtn.addEventListener('click', function() {
                            openOpportunityDetails(opp.title, {
                                duration: opp.duration,
                                startDate: opp.startDate,
                                description: opp.description,
                                skills: skills,
                                applicants: opp.maxApplicants || 0,
                                futureOpportunities: opp.futureOpportunities,
                                prerequisites: ''
                            });
                        });
        ...
        function editOpportunity(id) {
            ...
            .then(data => {
                if(data.status === 'success') {
                    ...
                    document.getElementById('oppName').value = opp.title;
                    document.getElementById('oppDuration').value = opp.duration;
                    document.getElementById('oppStartDate').value = opp.startDate;
                    ...
                    document.getElementById('oppFuture').value = opp.futureOpportunities;
                    if(document.getElementById('oppMaxApplicants')) {
                        document.getElementById('oppMaxApplicants').value = opp.maxApplicants || '';
                    }
